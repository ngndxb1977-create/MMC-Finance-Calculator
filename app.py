...
st.markdown(
 ...
 <style>
 @import 
url('https://fonts.googleapis.com/css2?family=Karma:wght@400;600&family=Amethysta&family=Quicksand:w
ght=500;700&display=swap');
 .stApp {
 ...
 font-size: 2.25rem !important;  color: #191919 !important;
...
def generate_html_report(
    ...
    """Generates a complete standalone HTML document containing all report details and vehicle image."""
    img_tag = (
        f'<img src="{img_b64}" style="max-width: 300px; height: auto; display: block; margin: 0 auto 20px auto;" />'
        if img_b64
        else ""
    )
    ...
    for r in vehicle_emi_results:
        emi_rows += f"""
        <tr>
            <td>{r["Asset Term (Years)"]}</td>
            <td>{r["Flat ROI %"]}</td>
            <td>{r["Principal Loan Block"]}</td>
            <td>{r["Total Interest Accrued"]}</td>
            <td><b>{r["Monthly Vehicle EMI"]}</b></td>
        </tr>
        """
    ...
    if finance_dp_option and dp_results:
        ...
        for r in dp_results:
            ...
            <tr>
                <td>{r["Term"]}</td>
                <td>{r["Financed Balance"]}</td>
                <td>{r["Total Interest"]}</td>
                <td><b>{r["Monthly EMI"]}</b></td>
            </tr>
            """
        dp_section_html = f"""
        <h3>Down Payment Loan Financing Options</h3>
        <table>
            <tr><th>Term</th><th>Financed Balance</th><th>Total Interest</th><th>Monthly EMI</th></tr>
            {dp_rows}
        </table>
        """
    ...
    if addons_table_data:
        for a in addons_table_data:
            addons_rows += f"""
            <tr>
                <td>{a["Selected Accessories / Services"]}</td>
                <td>{a["Individual Price (Base)"]}</td>
                <td>{a["VAT Amount (5%)"]}</td>
                <td>{a["Total Cost (incl. VAT)"]}</td>
            </tr>
            """
    else:
        addons_rows = (
            '<tr><td colspan="4">No optional accessories selected.</td></tr>'
        )
    ...
    <head>
        <meta charset="utf-8">
        <title>Mitsubishi Financial Report - {selected_name} ({selected_code})</title>
        <style>
            body {{ font-family: Arial, sans-serif; color: #191919; line-height: 1.5; padding: 20px; max-width: 900px; margin: auto; background: #fff; }}
            h1, h2, h3 {{ color: #191919; font-family: 'Times New Roman', serif; }}
            h1 {{ text-align: center; border-bottom: 2px solid #191919; padding-bottom: 10px; }}
            table {{ width: 100%; border-collapse: collapse; margin-top: 15px; margin-bottom: 25px; }}
            th, td {{ border: 1px solid #ddd; padding: 10px; text-align: left; font-size: 14px; }}
            th {{ background-color: #F4F0EA; color: #191919; }}
            .metrics-container {{ display: flex; justify-content: space-between; margin-bottom: 20px; }}
            .metric-box {{ background: #F4F0EA; padding: 15px; border-radius: 6px; width: 30%; text-align: center; }}
            .outlay-box {{ background-color: #F4F0EA; padding: 20px; border-radius: 8px; border-left: 4px solid #191919; margin-top: 20px; }}
    ...
    <body>
        ...
        <h2 style="text-align: center;">Unit Selected: {selected_name} — Variant {selected_code} ({selected_year})</h2>
        {img_tag}
        
        <h2>1. Base Vehicle Price & Tax Breakdown</h2>
        <table>
            <tr><th>Vehicle Model / Variant</th><th>Base Unit Price (Excl. VAT)</th><th>VAT (5%)</th><th>Total Unit Price (Incl. VAT)</th></tr>
            <tr>
                <td>{selected_name} ({selected_code})</td>
                <td>{base_vehicle_price:,.2f} AED</td>
                <td>{base_vehicle_vat:,.2f} AED</td>
                <td><b>{total_base_vehicle_price:,.2f} AED</b></td>
            </tr>
        </table>
        ...
        <table style="width: 100%; border: none;">
            <tr>
                <td style="background: #F4F0EA; text-align: center;"><b>Total Vehicle Value (incl. Addons)</b><br>{full_vehicle_value_including_addons:,.2f} AED</td>
                <td style="background: #F4F0EA; text-align: center;"><b>Gross Down Payment Req.</b><br>{calculated_downpayment:,.2f} AED</td>
                <td style="background: #F4F0EA; text-align: center;"><b>Vehicle Finance Amount</b><br>{finance_amount:,.2f} AED</td>
            </tr>
        </table>
        ...
        <table>
            <tr><th>Asset Term (Years)</th><th>Flat ROI %</th><th>Principal Loan Block</th><th>Total Interest Accrued</th><th>Monthly Vehicle EMI</th></tr>
            {emi_rows}
        </table>
        {dp_section_html}
        ...
        <table>
            <tr><th>Selected Accessories / Services</th><th>Individual Price (Base)</th><th>VAT Amount (5%)</th><th>Total Cost (incl. VAT)</th></tr>
            {addons_rows}
        </table>
        ...
        <table>
            <tr>
                <td><b>Registration Fee:</b> {registration_fee:,.2f} AED<br><b>DP Processing Fee:</b> {dp_processing_fee:,.2f} AED<br><b>Bank Processing Fee:</b> {bank_processing_fee:,.2f} AED</td>
            </tr>
        </table>
        <div class="outlay-box">
            <span style="text-transform: uppercase; font-size: 12px; color: #555;">Actual Upfront Cash Required at Showroom Handover</span>
            <div style="font-size: 24px; font-weight: bold; color: #191919;">{grand_total_cash_outlay:,.2f} AED</div>
        </div>
        ...
        <div class="disclaimer">
            <b>Disclaimer:</b> All calculations, rates, and figures provided by this matrix calculator are for estimation purposes only and subject to formal bank approval, final credit evaluation, and prevailing regulatory changes in the UAE.
        </div>
    </body>
    </html>
    """
    ...
    if os.path.exists(file_path):
        try:
            df_bank = pd.read_excel(
                file_path, sheet_name="Bank Details", header=None
            )
        ...
        try:
            ...
            for _, row in df_rmc.iterrows():
                ...
                rmc_data[v_code] = {
                    "RMC-10-40": (
                        float(row["RMC-10-40"])
                        if pd.notna(row["RMC-10-40"])
                        else 0.0
                    ),
                    "RMC-10-60": (
                        float(row["RMC-10-60"])
                        if pd.notna(row["RMC-10-60"])
                        else 0.0
                    ),
                    "RMC-10-70": (
                        float(row["RMC-10-70"])
                        if pd.notna(row["RMC-10-70"])
                        else 0.0
                    ),
                    "RMC-10-100": (
                        float(row["RMC-10-100"])
                        if pd.notna(row["RMC-10-100"])
                        else 0.0
                    ),
                }
        except:
    ...
    for sheet in xls.sheet_names:
        ...
        try:
            ...
            if not code or code == "nan":
                code = sheet
            year_key = (
                "2026" if "2026" in year_string or "26" in sheet else "2025"
            )
            ...
            for row_idx, default_label in row_labels.items():
                ...
                label = (
                    cell_label
                    if (cell_label and cell_label != "nan")
                    else default_label
                )
...
if not VEHICLE_CATALOG["2025"] and not VEHICLE_CATALOG["2026"]:
    ...
    with st.sidebar:
        ...
        if selected_name:
            ...
            v_data = VEHICLE_CATALOG[selected_year][selected_name][
                selected_code
            ]
        else:
            st.stop()
        ...
        if resolved_path:
            ...
            st.sidebar.error(
                f"Missing: Put '{img_file}' in your 'images' folder."
            )
        ...
        if BANK_RULES:
            ...
            selected_bracket = st.selectbox(
                "Income Bracket Selection:", bracket_options
            )
        ...
        down_payment_pct = (
            st.slider("Down Payment Percentage (%):", 0, 100, 20) / 100.0
        )
        ...
        st.markdown("---")
        st.subheader("Custom Accessories & Packages")
        ...
        if EXTERNAL_ACCESSORIES:
            selected_acc_names = st.multiselect(
                ...
                default=[],
                help=(
                    "Search and select multiple accessories and packages from"
                    " your catalog."
                ),
            )
            for acc_item in selected_acc_names:
                ...
                if "WARRANTY" in item_name_upper:
                    ...
                    "FOPPF" in item_name_upper
                    or "GOLD PACKAGE" in item_name_upper
                ):
                ...
                if info["type_tag"] == "STANDARD":
                    ...
                    if checked:
                        if "WARRANTY" in name.upper():
                            ...
                            "CUSTOM" in name.upper()
                            and "ACCESSORIES" in name.upper()
                        ):
                            acc_selected_price += info["price_raw"]
                        elif (
                            "CERAMIC" in name.upper()
                            and "WINDOW" in name.upper()
                        ):
        ...
        vehicle_insurance_cost = (
            display_ins_price if is_insurance_selected else 0.0
        )
        ...
        finance_amount = (
            full_vehicle_value_including_addons - calculated_downpayment
        )
...
if st.session_state.view_state == "input":
    ...
    st.markdown(
        f"""
 <div style="background-color: #F4F0EA; padding: 1.25rem 1.5rem; border-radius: 8px; border-left: 4px solid 
#191919; margin-top: 1.5rem;">
 <span style="font-family: 'Quicksand', sans-serif; font-weight: 700; font-size: 0.9rem; color: #555555; 
display: block; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 0.25rem;">
 ...
 """,
    ...
    # DOWNLOAD HTML / PRINT REPORT ACTION
    ...
    html_report_str = generate_html_report(
        ...
        addons_table_data=addons_table_data,
    ...
    st.download_button(
        label="📥 Download Report as HTML (Printable to PDF)",
        data=html_report_str,
        file_name=f"Mitsubishi_Financial_Report_{selected_name}_{selected_code}.html",
        mime="text/html",
        use_container_width=True,
    )
    st.caption(
        "💡 *Tip: Open the downloaded HTML file in any web browser and press `Ctrl+P` (or `Cmd+P`) to save/print it directly as a clean PDF document.*"
    )
    ...
    st.markdown(
        """
 <div style="background-color: #F4F0EA; padding: 1.5rem; border-radius: 8px; border-left: 4px solid 
#191919; margin-top: 1.5rem;">
 <strong style="font-family: sans-serif; font-size: 1.1rem; color: #191919; display: block; margin-bottom: 
0.75rem;"> Required Documentation Checklist:</strong>
 <ul style="margin-bottom: 1rem; padding-left: 1.25rem;">
 <li>Passport Copy, Digital Visa & Address Page For Indian Passport, Page #44 For Philippines 
Passport.</li>
 <li>Emirates ID Card Copy Both Sides.</li>
 <li>Labour Card / Free Zone / Employer ID.</li>
 <li>Copy of the UAE Driver's License Both Sides.</li>
 <li>Current Dated Salary Certificate from The Employer.</li>
 <li>Pay Slips For The Last 3 Months (Or 6 Months For Commission-Based Income).</li>  <li>Latest 3 to 6 Months Personal Bank Statement showing Salary Credits.</li>
 </ul>
 <div class="disclaimer-text">
 <strong>Disclaimer:</strong> All calculations, rates, and figures provided by this matrix calculator are 
for estimation purposes only and subject to formal bank approval, final credit evaluation, and prevailing 
